...
fn create_planning_documents(vault_path: &Path) -> Result<(), String> {
    ...
    for doc in PLANNING_DOCUMENTS {
        ...
        for header in doc.required_headers {
            ...
            content.push_str("*[Content placeholder]*\n\n\n\n");
        }

        content.push_str("---\n");
        content.push_str("*Generated by Nexus CLI*\n");
        content.push_str(&format!("*{}*\n", doc.template_guidance));

        fs::write(planning_dir.join(doc.filename), content)
            .map_err(|e| format!("Failed to write {}: {e}", doc.filename))?;

        println!("    {} {}", "✓".green(), doc.filename);
    }
