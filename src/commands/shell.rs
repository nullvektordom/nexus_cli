...
pub fn execute() -> Result<()> {
    ...
    loop {
        ...
        let prompt = if let Some(ref project) = state.active_project_id {
            let watch_indicator = if *watcher_enabled.lock().unwrap() { "👁" } else { "" };
            format!("{} ", format!("nexus:{}{}❯", project, watch_indicator).cyan().bold())
        } else {
        ...
        match readline {
            Ok(line) => {
                ...
                if let Err(e) = execute_command(line, &mut state, &watcher, &watcher_enabled, &last_gate_error) {
            ...
            Err(ReadlineError::Interrupted) => {
                println!("{}", "^C - Use 'exit' or 'quit' to leave the shell".yellow());
                continue;
            }
...
fn print_banner(state: &NexusState) -> Result<()> {
    println!("{}", "╔═══════════════════════════════════════════════════════╗".cyan());
    println!("{}", "║        🧠 Nexus Shell - Interactive REPL           ║".cyan());
    println!("{}", "╚═══════════════════════════════════════════════════════╝".cyan());
    println!();
    println!("  {} {}", "Session ID:".bold(), state.session_id.dimmed());
    println!("  {} {}", "Obsidian Root:".bold(), state.obsidian_vault_root.display().to_string().dimmed());
    println!("  {} {}", "Repos Root:".bold(), state.repos_root.display().to_string().dimmed());

    if let Some(ref project) = state.active_project_id {
        println!("  {} {}", "Active Project:".bold(), project.green());
    } else {
        println!("  {} {}", "Active Project:".bold(), "None (use 'use <project>' to select)".yellow());
    }

    println!("  {} {}", "Started:".bold(), state.created_at.dimmed());
    println!();
    println!("{}", "Available commands: use, gate, unlock, sprint, status, help, exit".dimmed());
...
fn execute_command(
    ...
    match command.as_str() {
        ...
        "use" => {
            execute_use(state, args)
        }
        "gate" => {
            execute_gate(state, last_gate_error)
        }
        "unlock" => {
            execute_unlock(state)
        }
        "sprint" => {
            execute_sprint(state, args)
        }
        "status" => {
            execute_status(state)
        }
        "watch" => {
            execute_watch(state, watcher, watcher_enabled)
        }
        "unwatch" => {
            execute_unwatch(watcher, watcher_enabled)
        }
        "why" => {
            execute_why(state, last_gate_error)
        }
        "clear" | "cls" => {
        ...
        "pwd" => {
            if let Some(ref project) = state.active_project_id {
                ...
                println!("{}", "No active project. Use 'use <project>' to select one.".yellow());
            }
            Ok(())
        }
        _ => {
...
fn print_help() {
    ...
    println!();
    println!("  {}  <project>   Select and activate a project", "use".cyan());
    println!("  {}         Check if planning documents are complete", "gate".cyan());
    println!("  {}       Generate CLAUDE.md from planning documents", "unlock".cyan());
    println!("  {}  <N>   Create/switch to sprint N", "sprint".cyan());
    println!("  {}       Check Brain health and memory usage", "status".cyan());
    println!("  {}          Show current active project and paths", "pwd".cyan());
    println!();
    println!("  {}        Start watching for file changes (Sentinel)", "watch".cyan());
    println!("  {}      Stop watching for file changes", "unwatch".cyan());
    println!("  {}          Explain last gate failure using Architecture", "why".cyan());
    println!();
    println!("  {}         Show this help message", "help".cyan());
    println!("  {}        Clear the screen", "clear".cyan());
    println!("  {}  | {}  Exit the shell", "exit".cyan(), "quit".cyan());
    println!();
    println!("{}", "  💡 Tip: Any unrecognized input is treated as a semantic search!".dimmed());
    println!();
}
...
fn execute_use(state: &mut NexusState, args: &[&str]) -> Result<()> {
    ...
    println!("{}", format!("Selecting project '{}'...", project_id).dimmed());
    ...
    println!("{} Project '{}' activated", "✓".green().bold(), project_id.green());

    if let Some(repo_path) = state.get_active_repo_path() {
        println!("  {} {}", "Repository:".bold(), repo_path.display().to_string().dimmed());
    }
    if let Some(obsidian_path) = state.get_active_obsidian_path() {
        println!("  {} {}", "Obsidian:".bold(), obsidian_path.display().to_string().dimmed());
    }
...
fn execute_gate(state: &NexusState, last_gate_error: &Arc<Mutex<Option<String>>>) -> Result<()> {
    let project_path = state.get_active_repo_path()
...
fn execute_sprint(state: &NexusState, args: &[&str]) -> Result<()> {
    ...
    println!("{}", format!("Activating sprint {}...", sprint_number).dimmed());
...
fn execute_status(state: &NexusState) -> Result<()> {
    ...
    println!("  {} {}", "Qdrant URL:".bold(), brain_config.qdrant_url.dimmed());
    println!();
    ...
    // Print health status
    println!("{}", "╔══════════════════════════════════════════════════╗".green());
    println!("{}", "║          🧠 Brain Status: ONLINE ✓             ║".green());
    println!("{}", "╚══════════════════════════════════════════════════╝".green());
    println!();

    let (ram, disk) = health.format_memory();

    println!("  {} {}", "Collection:".bold(), health.collection_name.green());
    println!("  {} {}", "Total Points:".bold(), health.points_count.to_string().cyan());
    println!("  {} {}", "Total Vectors:".bold(), health.vectors_count.to_string().cyan());
    println!("  {} {}", "Indexed Vectors:".bold(), health.indexed_vectors_count.to_string().cyan());
    println!("  {} {}", "Segments:".bold(), health.segments_count.to_string().cyan());
    println!();
...
fn execute_watch(
    ...
    *watcher_enabled.lock().unwrap() = true;
    ...
    println!("{} Sentinel is now watching for changes", "✓".green().bold());
...
fn execute_why(state: &NexusState, last_gate_error: &Arc<Mutex<Option<String>>>) -> Result<()> {
    ...
    if results.is_empty() {
        println!("{}", "No relevant architecture documentation found.".yellow());
        return Ok(());
    }
    ...
    for (idx, result) in results.iter().enumerate() {
        println!("{} {} {}", format!("[{}]", idx + 1).cyan(), "From".dimmed(), result.file_name().green());
        println!("   {} {}", "Score:".dimmed(), format!("{:.2}", result.score).yellow());
        println!("   {}", result.content.dimmed());
        println!();
    }

    println!("{}", "💡 These architecture guidelines may help resolve the issue.".cyan());
...
fn execute_semantic_query(query: &str, state: &NexusState) -> Result<()> {
    let project_id = state.active_project_id.as_ref()
        .ok_or_else(|| anyhow::anyhow!("No active project. Use 'use <project>' first for semantic search."))?;

    let repo_path = state.get_active_repo_path()
        .ok_or_else(|| anyhow::anyhow!("Failed to get repo path"))?;

    println!("{} \"{}\"", "🔍 Searching Brain for:".cyan(), query.italic());
    ...
    println!();
    println!("{} {} {}", "Found".green().bold(), results.len(), "results:".green().bold());
    println!();
    ...
    for (idx, result) in results.iter().enumerate() {
        ...
        println!("   {} {}", "Relevance:".dimmed(), format!("{:.1}%", result.score * 100.0).yellow());
